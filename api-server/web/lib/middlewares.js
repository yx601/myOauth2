...
var logger=log4js.getLogger('middlewares');

// 检查用户是否已登录
exports.ensureLogin = function (req, res, next) {
  // 这里直接设置用户ID=glen
  if(!req.session.user){
      let params={redirectUri:req.url,};
      let url=utils.addQueryParamsToUrl('/OAuth2/login',params);
      logger.info(url);
      return res.redirect(url);
  }
  req.loginUserId=req.session.user;
  next();
};
